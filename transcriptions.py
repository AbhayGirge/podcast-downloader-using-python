...
def save_transcriptions_locally(podcast_list):
    ...
    for podcast in podcast_list:
        podcast_transcriptions = metadata.get(podcast.name, {})
        for episode, transcription_id in podcast_transcriptions.items():
            ...
            if 'text' in transcription and transcription['text'] is not None:
                with open(output_path, 'w') as f:
                    f.write(transcription['text'])
                print('Transcription saved successfully:', output_path)
            else:
                print('Error: Transcription text is None or not available for', output_path)
...
def wait_and_get_assembly_ai_transcript(transcription_id):
	while True:
		...
		if response['status'] == 'completed':
			...
			print("Transcript not available, trying again in 2 minutes...")
			time.sleep(100) # Try again in 5 minutes
